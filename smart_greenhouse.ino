...
const unsigned long PUMP_RUN_TIME = 2000;          // 2 секунд работы насоса
...
void setup() {
  ...
  ventServo.attach(SERVO_PIN);      // <--- ЭТОЙ СТРОКИ НЕ ХВАТАЕТ!
  ventServo.write(SERVO_CLOSED);    // Закрыто (0°)
  delay(500);                       // Ждем пока повернется
  
  // ----- ШАГ 9: ВЫВОД ИНФОРМАЦИИ О НАСТРОЙКАХ -----
  Serial.println("\n=== SYSTEM SETTINGS ===");
  ...
  // ----- ШАГ 10: ТЕСТИРОВАНИЕ РЕЛЕ ДЛЯ ФИТОЛЕНТЫ -----
  ...
  // ----- ШАГ 11: ВЫВОД СТАТУСА ВСЕХ ДАТЧИКОВ -----
  Serial.println("\n=== SENSOR STATUS ===");
  ...
  // ----- ШАГ 12: ПОКАЗ ПРИВЕТСТВЕННОГО СООБЩЕНИЯ НА ДИСПЛЕЕ -----
...
void controlWindow(float temperature) {
  ...
  // Проверяю, не пора ли открыть форточку: 
  // 1. Температура выше или равна порогу открытия (28°C)
  // 2. Форточка сейчас закрыта (!windowIsOpen означает "НЕ открыта")
  if (temperature >= OPEN_TEMP && !windowIsOpen) {
  
    // Даю команду сервоприводу повернуться на угол OPEN (90 градусов)
    ventServo.write(SERVO_OPEN);
  
    // Жду 500 миллисекунд (полсекунды), чтобы сервопривод успел повернуться
    // Без этой задержки он может не успеть выполнить команду
    delay(500);
  
    // Запоминаю текущий угол поворота (для информации)
    ventAngle = SERVO_OPEN;
  
    // Запоминаю, что форточка теперь открыта
    // Это нужно, чтобы не посылать команду снова и снова
    windowIsOpen = true;
  
    // Вывожу в Serial Monitor сообщение об открытии
    Serial.print("VENTILATION: Opening window at ");
    Serial.print(temperature);
    Serial.println("C (too hot)");
  
    // Если дисплей работает в нормальном режиме
    if (currentDisplayMode == DISPLAY_NORMAL) {
      // Очищаю экран
      lcd->clear();
      // Печатаю предупреждение о высокой температуре
      safePrint("TEMPERATURE HIGH", 0, 0);
      // Печатаю сообщение об открытии форточки
      safePrint("OPENING WINDOW", 0, 1);
      // Держу сообщение на экране 1 секунду
      delay(1000);
  }
} 

  // УСЛОВИЕ 2: ЗАКРЫТЬ ФОРТОЧКУ
  // Проверяю, не пора ли закрыть форточку:
  // 1. Температура ниже или равна порогу закрытия (26°C)
  // 2. Форточка сейчас открыта
  else if (temperature <= CLOSE_TEMP && windowIsOpen) {
  
    // Даю команду сервоприводу вернуться в закрытое положение (0 градусов)
    ventServo.write(SERVO_CLOSED);
  
    // Жду полсекунды, чтобы сервопривод успел закрыться
    delay(500);
  
    // Запоминаю текущий угол (0 градусов)
    ventAngle = SERVO_CLOSED;
  
    // Запоминаю, что форточка теперь закрыта
    windowIsOpen = false;
  
    // Вывожу в Serial Monitor сообщение о закрытии
    Serial.print("VENTILATION: Closing window at ");
    Serial.print(temperature);
    Serial.println("C (normal temp)");
  
    // Если дисплей работает
    if (currentDisplayMode == DISPLAY_NORMAL) {
      // Очищаю экран
      lcd->clear();
      // Печатаю сообщение о нормальной температуре
      safePrint("TEMPERATURE OK", 0, 0);
      // Печатаю сообщение о закрытии форточки
      safePrint("CLOSING WINDOW", 0, 1);
      // Держу сообщение 1 секунду
      delay(1000);
  }
}
...
void debugOutput() {
  ...
  if (millis() - lastDebug > 5000) {
    ...
    // Вывод состояния датчика воды для отладки
    Serial.print("Water sensor (HCW-M203): ");
    Serial.print(digitalRead(WATER_SENSOR_PIN) ? "HIGH" : "LOW");
    Serial.print(" → Water: ");
    Serial.println(waterPresent ? "FULL" : "EMPTY");
