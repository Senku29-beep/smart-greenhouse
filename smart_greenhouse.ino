...
void setup() {
  ...
  // ----- ШАГ 8: НАСТРОЙКА АНАЛОГОВЫХ ВХОДОВ -----
...
void readSensors() {
 ...
 // 4. ЧТЕНИЕ HCW-M203 (датчик уровня воды через реле)
  #if ENABLE_WATER
  ...
  // Читаем состояние реле HCW-M203
  // Когда вода есть - реле замкнуто, на пине LOW
  // Когда воды нет - реле разомкнуто, на пине HIGH (благодаря PULLUP)
  bool waterDetected = (digitalRead(WATER_SENSOR_PIN) == HIGH);
  
  // Сохраняем в глобальную переменную
  waterPresent = waterDetected;
  ...
  #else
    // Если датчик отключен - считаем что вода есть
    waterPresent = true;
  #endif
}
...
void controlWindow(float temperature) {
  ...
  ventServo.attach(SERVO_PIN);   // Подключаем (если был отключён)
  delay(15);
  ventServo.write(SERVO_OPEN);
  delay(500);
  ventServo.detach();             // <--- ОТКЛЮЧАЕМ
  ...
  ventServo.attach(SERVO_PIN);   // Подключаем
  delay(15);
  ventServo.write(SERVO_CLOSED);
  delay(500);
  ventServo.detach();             // <--- ОТКЛЮЧАЕМ
